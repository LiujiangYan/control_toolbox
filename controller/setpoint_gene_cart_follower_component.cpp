/*
 * setpoint_generator_cart_following.cpp
 *
...
using namespace RTT;
using namespace KDL;
SetpointGeneratorCartFollowingComponent::SetpointGeneratorCartFollowingComponent(const string& name, double interpPeriod,
...
SetpointGeneratorCartFollowingComponent::~SetpointGeneratorCartFollowingComponent() {
}

bool SetpointGeneratorCartFollowingComponent::initialize(){
	return true;
}

int SetpointGeneratorCartFollowingComponent::Interpolate(double *pos_data) {
	int res = 0;
	...
	if(!m_command->GetElement(m_cart_command)){
...
//			Logger::log()<<Logger::Info<<"RosController's Interpolator Started, goal is "<<m_command_state.position[0]<<Logger::endl;
			m_last_command_time = m_cart_command.header.stamp - ros::Duration(m_command_period);
			/*
			 * 记录开始时间，以此作为comand时间轴和插补时间轴的共有起点。
			 */
			m_start_command_time = m_cart_command.header.stamp - ros::Duration(m_command_period);
//			cout<<"m_start_command_time = "<<m_start_command_time<<endl;
	...
	*/
		m_duration_since_start_cmd = m_cart_command.header.stamp - m_start_command_time;
...
//		Logger::log()<<Logger::Info<<"m_plan_duration = "<<m_plan_duration<<Logger::endl;
		...
		m_last_command_time = m_cart_command.header.stamp;

		PoseToFrame(m_cart_command.pose,m_command_frame);
		res = m_iksolver->CartToJnt(m_this_state.q,m_command_frame,m_goal_state.q);
		if(res != 0){
			Logger::log()<<Logger::Info<<"笛卡尔空间指令点逆解失败 "<<Logger::endl;
//					<<m_command_frame.m_origin.m_x<<","<<m_command_frame.m_origin.m_y<<","<<m_command_frame.m_origin.m_z<<","<<Logger::endl;
//			Logger::log()<<Logger::Info<<"笛卡尔空间指令点逆解失败: Orientation = "
//					<<m_command_frame.m_orient.m_Xx<<","
//					<<m_command_frame.m_origin.m_x<<","<<m_command_frame.m_origin.m_y<<","<<m_command_frame.m_origin.m_z<<","<<Logger::endl;
//
			stringstream errorInfo;
			errorInfo<<"笛卡尔空间指令点逆解失败";
			GotoErrorState("50050", errorInfo.str());
		}

		m_goal_state.qdot.data = (m_goal_state.q.data - m_last_goal_pos.data)/m_command_period;
		m_last_goal_pos = m_goal_state.q;
...
void SetpointGeneratorCartFollowingComponent::finalize(){

}
void SetpointGeneratorCartFollowingComponent::reset(){
	m_count = 0;
	m_started = false;
	m_commanded_stop = false;
...
void SetpointGeneratorCartFollowingComponent::stop(){
	m_commanded_stop = true;
}
/*
 * 判断是否位置超限
 */
bool SetpointGeneratorCartFollowingComponent::IsJntPosWithinLimits(const KDL::JntArray& q, unsigned int& index, double eps){
	for(unsigned int i=0;i<m_joint_num;i++){
 ...
 * 轴速度超限
 */
bool SetpointGeneratorCartFollowingComponent::IsJntVelWithinLimits(const KDL::JntArray& qdot, unsigned int& index, double eps){
	for(unsigned int i=0;i<m_joint_num;i++){
 ...
 * 轴加速超限
 */
bool SetpointGeneratorCartFollowingComponent::IsJntAccWithinLimits(const KDL::JntArray& qdotdot, unsigned int& index, double eps){
	for(unsigned int i=0;i<m_joint_num;i++){
...
void SetpointGeneratorCartFollowingComponent::GotoErrorState(const string error_code, const string arglist)
{
